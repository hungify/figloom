...
describe("runWithConcurrency", () => {
  it("returns results in input order even when later items resolve first", async () => {
    const results = await runWithConcurrency([10, 40, 10], 3, (delayMs, index) => deferred(index, delayMs));
  ...
  it("runs concurrently rather than sequentially when limit allows it", async () => {
    ...
    const overlapping = spans.some((a) => spans.some((b) => a !== b && a.start < b.end && b.start < a.end));
    expect(overlapping).toBe(true);
  });

  it("clamps limit to item count and to a minimum of 1", async () => {
    await expect(runWithConcurrency([1, 2, 3], 100, (item) => Promise.resolve(item))).resolves.toEqual([1, 2, 3]);
    await expect(runWithConcurrency([1, 2, 3], 0, (item) => Promise.resolve(item))).resolves.toEqual([1, 2, 3]);
    await expect(runWithConcurrency([1, 2, 3], Number.NaN, (item) => Promise.resolve(item))).resolves.toEqual([1, 2, 3]);
  });
