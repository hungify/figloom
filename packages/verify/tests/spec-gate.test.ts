...
describe("spec gate (Figma REST spec vs DOM size)", () => {
  ...
  it("node without absoluteBoundingBox -> skip with warning", async () => {
    const fetchImpl = (async () =>
      new Response(
        JSON.stringify({ nodes: { "153:5181": { document: { type: "GROUP" } } } }),
        { status: 200 },
      )) as typeof fetch;
    const r = await specGate({ ...base, domSize: { width: 1, height: 1 }, fetchImpl });
    expect(r.pass).toBeNull();
    expect(r.warnings[0]).toMatch(/no absoluteBoundingBox/);
  });
