...
describe("legacy Figma run contract guards", () => {
  it("invalidates old verdict before rejected run", async () => {
    ...
    if (!result.ok) expect(result.error).toBe("SELECTOR_REQUIRED");
    expect(fs.existsSync(path.join(outDir, "visual-score.json"))).toBe(false);
  });

  it("returns a structured rejection when conventional gold is missing", async () => {
    const outDir = path.join(tmp, "missing-gold");
    const result = await run(base(outDir));
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.error).toBe("GOLD_NOT_FOUND");
  });
