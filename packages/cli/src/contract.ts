...
export async function runCreateContract(options: {
  ...
  p.intro("Create Figloom visual contract");
  ...
  const expectedUrl = await text({
    message: "Expected URL after Playwright navigation",
    initialValue: targetUrl,
    validate: validateHttpUrl,
  });
  if (!expectedUrl) return;

  const readySelectorInput = await p.text({
    message: "Screen readiness selector (optional)",
    placeholder: "[data-testid=login-ready]",
  });
  if (cancelled(readySelectorInput)) return;
  const readySelector = readySelectorInput?.trim() || undefined;

  const auth = await select<"none" | "storageState">({
    message: "Authentication",
    options: [
      { value: "none", label: "No saved session", hint: "login and public pages" },
      { value: "storageState", label: "Saved Playwright session", hint: "protected pages" },
    ],
  });
  if (!auth) return;
