...
import { toJsonSchema, verificationArtifactSchema, verificationRequestSchema } from "@figloom/contracts";
...
function schemaCommand(options: SchemaOptions): void {
  const schema = options.target === "artifact" ? verificationArtifactSchema : verificationRequestSchema;
  console.log(JSON.stringify(toJsonSchema(schema), null, JSON_INDENT_SPACES));
}

export function registerSchemaCommand(program: Command): void {
  program.addCommand(
    subcommand("schema", "Print the live JSON Schema for a visual contract or verification artifact.")
