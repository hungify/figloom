...
export function useRunDashboard() {
  ...
  const artifactMode = computed(() => resolveArtifactMode({
    mockMode: mockMode.value,
    staticMode: staticMode.value,
    liveMode: liveMode.value,
  }));

  const contracts = computed(() => filterContracts(run.value?.contracts ?? [], query.value, status.value));
  ...
  const selectedTreeItem = computed(() => findContractTreeItem(contractTree.value, selected.value?.id));
