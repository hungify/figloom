...
const apiOrigin = process.env.FIGLOOM_API_ORIGIN;

function dashboardMockPlugin(): Plugin {
  return {
    name: 'figloom-dashboard-mock',
    apply: 'serve',
    configureServer(server) {
      ...
      server.middlewares.use((request, response, next) => {
        ...
        if (pathname.startsWith('/artifacts/')) {
          const artifact = dashboardMockArtifacts[decodeURIComponent(pathname.slice('/artifacts/'.length))];
          if (artifact) {
...
export default defineConfig({
  ...
  build: {
    outDir: '../../packages/cli/dist/dashboard',
    emptyOutDir: true,
    sourcemap: true,
  },
  server: apiOrigin ? {
