...
function contract(partial: Partial<DashboardContractResult> & Pick<DashboardContractResult, 'id' | 'name' | 'status'>): DashboardContractResult {
  return {
    tags: [],
    phase: 'complete',
    baselineKind: 'web',
...
describe('filterContracts', () => {
  ...
  it('filters by status and query', () => {
    expect(filterContracts(items, '', 'failed').map((item) => item.id)).toEqual(['checkout.desktop']);
    expect(filterContracts(items, 'header', 'all').map((item) => item.id)).toEqual(['header.mobile']);
    expect(filterContracts(items, 'mobile', 'passed').map((item) => item.id)).toEqual(['header.mobile']);
  });
});
...
describe('buildContractTree', () => {
  it('groups by feature and aggregates worst status', () => {
    const tree = buildContractTree([
      contract({ id: 'checkout.desktop', name: 'Checkout Desktop', status: 'passed', feature: 'checkout' }),
      contract({ id: 'checkout.mobile', name: 'Checkout Mobile', status: 'failed', feature: 'checkout' }),
    ...
    expect(tree[0]?.kind === 'feature' && tree[0].children.map((child) => child.label)).toEqual(['Desktop', 'Mobile']);
    expect(tree[1]?.kind === 'feature' && tree[1].status).toBe('blocked');
  });

  it("uses viewport/name when id equals feature key", () => {
    const tree = buildContractTree([
      contract({ id: 'checkout', name: 'Checkout Root', status: 'passed', feature: 'checkout', tags: ['desktop'] }),
    ]);
