...
describe('resolveArtifactMode', () => {
  ...
  it('resolves report, live, archived', () => {
    expect(resolveArtifactMode({ mockMode: false, staticMode: true, liveMode: false })).toBe('report');
    expect(resolveArtifactMode({ mockMode: false, staticMode: false, liveMode: true })).toBe('live');
    expect(resolveArtifactMode({ mockMode: false, staticMode: false, liveMode: false })).toBe('archived');
  });
});
