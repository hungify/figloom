...
const VERDICT_PRIORITY: DashboardVerdict[] = ['failed', 'blocked', 'running', 'queued', 'passed'];
...
function contractChildLabel(contract: DashboardContractResult, normalizedFeatureKey: string): string {
  const idLower = contract.id.toLowerCase();
  const prefix = `${normalizedFeatureKey}.`;
  const suffix = idLower === normalizedFeatureKey
    ? ""
    : idLower.startsWith(prefix)
      ? contract.id.slice(prefix.length)
      : contract.id;
  if (suffix) return suffix.split(".").map((part) => titleCase(part)).join(" · ");
...
function aggregateStatus(contracts: DashboardContractResult[]): DashboardVerdict {
  return VERDICT_PRIORITY.find((verdict) => contracts.some((contract) => contract.status === verdict)) ?? 'passed';
}
